...
interface ExpenseBreakdownProps {
  data: {
    currentPeriod: {
      ...
      netProfit: number;
      returnsAmount?: number; // Добавляем опциональное поле для суммы возвратов
    };
  };
...
const ExpenseBreakdown = ({ data, advertisingBreakdown }: ExpenseBreakdownProps) => {
  ...
  // Получаем сумму возвратов из данных, или 0 если не указано
  ...
  const totalExpensesWithCostPrice = data.currentPeriod.expenses.logistics + 
                                    ...
                                    totalCostPrice +
                                    returnsAmount; // Добавляем сумму возвратов в общие расходы
  ...
  return (
    <Card className="p-4">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-7 gap-2">
        ...
        {/* Добавляем блок с возвратами */}
        <div className="flex flex-col bg-gradient-to-br from-rose-50 to-white dark:from-rose-950/20 dark:to-background border border-rose-200 dark:border-rose-800 rounded-lg p-2">
          <div className="flex justify-between items-center mb-1">
            <h4 className="text-xs font-medium">Возвраты</h4>
            <div className="bg-rose-100 dark:bg-rose-900/60 p-1 rounded-md">
              <PackageX className="h-3 w-3 text-rose-600 dark:text-rose-400" />
            </div>
          </div>
