...
const StorageProfitabilityAnalysis: React.FC<StorageProfitabilityAnalysisProps> = ({
  ...
  // Функция для получения уникального идентификатора товара
  const getItemKey = (item: WarehouseRemainItem): string => {
    return item.vendorCode || item.supplierArticle || item.nmId.toString();
  };
  ...
  // Обновленные функции для работы с inputs с использованием itemKey
  const updateCostPrice = (itemKey: string, value: string) => {
    setCostPrices(prev => {
  ...
  const updateSellingPrice = (itemKey: string, value: string) => {
  ...
  const updateDailySales = (itemKey: string, value: string) => {
  ...
  const updateStorageCost = (itemKey: string, value: string) => {
  ...
  const updateLogisticsCost = (itemKey: string, value: string) => {
  ...
  const updateWbCommission = (itemKey: string, value: string) => {
