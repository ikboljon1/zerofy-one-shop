...
import { fetchAllProductData } from '@/services/wildberriesPricingApi';
...
const StorageProfitabilityAnalysis: React.FC<StorageProfitabilityAnalysisProps> = ({
    ...
    try {
      ...
      if (!apiKey) {
        toast({
          ...
          description: "Для загрузки данных через API необходимо указать API ключ Wildberries",
          variant: "destructive"
        });
        return;
      }
      ...
      try {
        ...
        toast({
          ...
          description: error.message || "Не удалось получить данные из API Wildberries",
          variant: "destructive"
        });
      }
    } finally {
  ...
  const loadDataFromWildberriesApi = async () => {
    ...
    await fetchSalesAndStorageData(analysisStartDate, analysisEndDate);
