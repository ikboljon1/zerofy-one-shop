...
const StorageProfitabilityAnalysis: React.FC<StorageProfitabilityAnalysisProps> = ({
  ...
  useEffect(() => {
    ...
    warehouseItems.forEach(item => {
      ...
      // Fix: Calculate storage cost properly from paid storage data
      let itemStorageCost = dailyStorageCost[nmId] || 5;
      
      // Find all storage entries for this nmId
      const matchingStorageItems = paidStorageData.filter(psi => psi.nmId === nmId);
      
      if (matchingStorageItems.length > 0) {
        // Calculate average daily storage cost from actual data
        const totalCost = matchingStorageItems.reduce((sum, psi) => sum + (psi.warehousePrice || 0), 0);
        const avgDailyCost = totalCost / Math.max(1, matchingStorageItems.length);
        itemStorageCost = Number(avgDailyCost.toFixed(2));
      }
