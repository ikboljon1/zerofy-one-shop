...
import { Package, ShoppingCart, TrendingUp, ShoppingBag, Warehouse } from "lucide-react";
...
const Chart = ({ salesTrend, productSales, storageCosts, chartType = 'line' }: ChartProps) => {
  const isMobile = useIsMobile();

  if (chartType === 'line' && !salesTrend) {
    return null;
  }
  
  if (chartType === 'pie' && !productSales) {
    return null;
  }
  
  if (chartType === 'bar' && !storageCosts) {
    return null;
  }
  ...
  if (productSales) {
    totalSales = productSales.reduce((sum, item) => sum + item.quantity, 0);
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Bar 
              ...
              fill="#8B5CF6" 
              activeBar={<Rectangle fill="#7C3AED" stroke="#fff" />}
            />
          </BarChart>
        </ResponsiveContainer>
      </div>
  ...
  return (
    <div className={`grid ${isMobile ? 'grid-cols-1 gap-6' : 'grid-cols-1 gap-4'}`}>
